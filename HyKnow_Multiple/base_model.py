...
class BaseModel(nn.Module):
    ...
    def decode_t(self, batch_size, u_input, u_hiddens, u_input_1hot, u_last_hidden, t_input, t_input_1hot,
                 turn_states, sample_type, context_to_response=False):

        t_prob, t_samples = [], []
        t_prob_ground, t_samples_ground = [], []
        log_pt = 0
        ...
        prev_tt = None
        prev_tt_ground = None
        pv_z_prob = turn_states.get('pv_pz_pr', None)
        pv_z_hiddens = turn_states.get('pv_pz_h', None)

        for t in range(self.t_length):
            prob, last_hidden, gru_out = \
            ...
            t_samples_ground.append(tt_ground.view(-1))
            t_prob_ground.append(prob_ground)

        t_prob = torch.stack(t_prob, dim=1)  # [B,Tt,V]
        t_samples = torch.stack(t_samples, dim=1)  # [B,Tt]
        t_prob_ground = torch.stack(t_prob_ground, dim=1)  # [B,Tt,V]
        t_samples_ground = torch.stack(t_samples_ground, dim=1)  # [B,Tt]

        return t_prob, t_samples, log_pt, t_prob_ground, t_samples_ground
    ...
    def decode_z_order(self, batch_size, u_input, u_hiddens, u_input_1hot, u_last_hidden, z_input, z_input_1hot,
                       pt_prob, pt_samples, pt_prob_ground, pt_samples_ground, turn_states, sample_type, decoder_type,
                       qz_samples=None, qz_hiddens=None,
        ...
        #dsv_prob, dsv_samples = [], []
        #Tt = pt_samples.size(1)
        #topic_mask_sample = cuda_(torch.zeros(pt_samples[:, 0].view(-1).size()))
        #topic_mask_prob = cuda_(torch.zeros(pt_prob[:, 0, :].squeeze(1).size()))
        log_dsv = 0
        ...
        #dsv_prob = torch.stack(dsv_prob, dim=1)
        #dsv_samples = torch.stack(dsv_samples, dim=1)
        z_prob = torch.stack(z_prob, dim=1)  # [B,Tz*K,V], K=len(self.reader.otlg.informable_slots)
        z_prob = torch.cat([z_prob, pt_prob], dim=1)
        z_prob_ground = torch.stack(z_prob_ground, dim=1)
        z_prob_ground = torch.cat([z_prob_ground, pt_prob_ground], dim=1)
        z_samples = torch.stack(z_samples, dim=1)  # [B,Tz*K]
        z_samples = torch.cat([z_samples, pt_samples], dim=1)
        z_samples_ground = torch.stack(z_samples_ground, dim=1)
        z_samples_ground = torch.cat([z_samples_ground, pt_samples_ground], dim=1)
