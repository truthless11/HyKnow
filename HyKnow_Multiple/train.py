...
from metric import MultiwozEvaluator
...
class Model:
    ...
    def _convert_batch(self, py_batch):
        ...
        t_input_np = utils.padSeqs(py_batch['topic'], maxlen=cfg.t_length, fixed_length=True)
        k_ground = py_batch['knowledge']
        ...
        t_input = torch.from_numpy(t_input_np).long()
        m_input = torch.from_numpy(m_input_np).long()
        if cfg.cuda:
            u_input, m_input, db_vec, t_input = u_input.cuda(), m_input.cuda(), db_vec.cuda(), t_input.cuda()
        ...
        return u_input, z_input, t_input, k_ground, a_input, m_input, db_vec, filling_vec, supervised


    def train(self, pretrain=False, skip_sup=False, ctr=False):
        self.final_epoch = 0
        ...
        # optim = torch.nn.DataParallel(optim, device_ids=[0, 1]).cuda()
        ...
        for epoch in range(max_epoch):
            ...
            for iter_num, dial_batch in enumerate(self.reader.batches['train']):
                ...
                for turn_num, turn_batch in enumerate(dial_batch):
                    ...
                    optim.zero_grad()
                    # here z denotes the dsv
                    u_input, z_input, t_input, k_ground, a_input, m_input, db_vec, filling_vec, supervised = \
                    ...
                    if supervised:
                        ...
                        losses, turn_states = self.model(u_input=u_input, z_input=z_input, m_input=m_input,
                                                         ...
                                                         a_input=a_input, context_to_response=ctr)
                        ...
                        losses, turn_states = self.model(u_input=u_input, z_input=z_input, m_input=m_input,
                                                         ...
                                                         no_label_train=cfg.no_label_train, context_to_response=ctr)
                        total_loss = losses['loss']
                        if cfg.prev_z_continuous:
                            total_loss.backward(retain_graph=turn_num!=len(dial_batch)-1)
                        else:
            ...
            valid_loss, valid_score = self.validate(ctr=ctr)
            train_loss_col.append(train_loss)
            valid_loss_col.append(valid_loss)
            valid_score_col.append(valid_score)

            logging.info('*************** RUNNING TEST CTR ***************')
            _, _ = self.validate(data='test', ctr=True)
            logging.info('*************** RUNNING TEST E2E ***************')
            _, _ = self.validate(data='test', ctr=False)
        ...
        if cfg.save_log and max_epoch != 0:   # Do evaluation and save result report
            ...
            if pretrain:
                ...
                logging.info("*************** Context to Response ***************")
                _ = self.eval(result_save_path=cfg.result_path_ctr, ctr=True)
                logging.info("*************** End to End ***************")
    ...
    def validate(self, data='dev', ctr=False):
        self.model.eval()
        result_collection = {}
        with torch.no_grad():
            loss_col, count = {}, 0
            ...
            for dial_batch in self.reader.batches[data]:
                ...
                for turn_num, turn_batch in enumerate(dial_batch):
                    ...
                    losses, turn_states_loss = self.model(u_input=u_input, z_input=z_input, m_input=m_input,
                                                          ...
                                                          a_input=a_input, db_vec=db_vec, filling_vec=filling_vec,
                                                          context_to_response=ctr)

                    for k, v in losses.items():
                        if k == 'reward': continue
                        loss_col[k] = v.item() if k not in loss_col else loss_col[k] + v.item()
                    count += 1
                    index, db, kb, turn_states = self.model(mode='test', u_input=u_input, z_input=z_input,
                                                            t_input=t_input, k_ground=k_ground,
            ...
            metrics = self.evaluator.run_metrics(results, ctr=ctr)
            combined = metrics['bleu'] + metrics['inform']/3 + metrics['joint_goal']
            score = {'bleu': metrics['bleu'], 'match': metrics['inform'], 'joint_goal': metrics['joint_goal'],
                          'combined': combined}
    ...
    def eval(self, data='test', result_save_path=None, ctr=False):
        result_collection = {}
        with torch.no_grad():
            self.model.eval()
        ...
        for k in loss_col:
            loss_col[k] /= (count + 1e-8)
        results, field = self.reader.wrap_result(result_collection)
        metric_results = self.evaluator.run_metrics(results, ctr=ctr)
...
def main():
    ...
    if args.mode in ['test', 'adjust', 'rl_tune']:
        ...
        if cfg.exp_path in ['', 'to be generated']:
            ...
            cfg.result_path_ctr = os.path.join(cfg.exp_path, 'result_ctr.csv')
    ...
    torch.backends.cudnn.benchmark = False  # do not use acceleration on conv algorithm to avoid output fluctuation
    torch.backends.cudnn.deterministic = True  # use default convolution algorithm to get deterministic outputs
    ...
    if args.mode == 'train':
        ...
        m.load_model(cfg.model_path)
        logging.info("*************** Context to Response ***************")
        _ = m.eval(result_save_path=cfg.result_path_ctr, ctr=True)
        logging.info("*************** End to End ***************")
