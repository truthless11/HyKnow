...
class SemiBootstrapSMC(BaseModel):
    ...
    def forward(self, u_input, m_input, t_input, k_ground, z_input, a_input, turn_states, z_supervised, mode,
                db_vec=None, filling_vec=None, no_label_train=False, context_to_response=False):
    ...
    def forward_turn(self, u_input, turn_states, is_train, m_input=None, z_input=None, t_input=None,
                     ...
                     no_label_train=False, context_to_response=False):
        """
        ...
        pt_prob, pt_samples, log_pt, pt_prob_ground, pt_samples_ground = \
        ...
        # z = dsv + t, P(dsv|pv_z, u) domain-slot-value decoding, as well as the combination and encoding of z
        pz_prob, pz_samples, z_hiddens, turn_states, pz_prob_ground, pz_samples_ground, z_hiddens_ground = \
            self.decode_z(batch_size, u_input, u_hiddens, u_input_1hot, u_last_hidden, z_input, z_input_1hot,
                            pt_prob, pt_samples, pt_prob_ground, pt_samples_ground,
                            turn_states, sample_type=sample_type, decoder_type='pz',
                            context_to_response=context_to_response)

        # KB Operation, need modified
        kb_matches = self.db_op.get_knowledge_match(pz_samples, self.vocab, self.eos_t_token, sample_type,
                                                    top_k=self.k_sample_size, context_to_response=context_to_response)
        ...
        if cfg.dataset == 'camrest':
            ...
            if context_to_response is not True:
        ...
        if not cfg.use_true_bspn_for_ctr_eval:
            ...
            pm_prob, m_idx, log_pm = \
                self.decode_m(batch_size, u_last_hidden, u_input, u_hiddens, u_input_1hot,
                              pz_samples_ground, pz_prob_ground, z_hiddens_ground, pa_samples, pa_prob, a_hiddens,
                              k_input_1hot, k_hiddens, db_vec, m_input, is_train=is_train)
        ...
        if is_train:
            ...
            t_idx = self.max_sampling(pt_prob) if not context_to_response else [list(_) for _ in list(t_input)]
