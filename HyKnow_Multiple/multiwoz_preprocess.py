...
def clean_time(utter):
        utter = re.sub(r'(\d+) ([ap]\.?m)', lambda x: x.group(1) + x.group(2), utter)   # 9 am -> 9am, 9 a.m -> 9am
        utter = re.sub(r'((?<!\d)\d:\d+)(am)?', r'0\1', utter)  # 3:16am -> 03:16, 8:23 -> 08:23
        utter = re.sub(r'((?<!\d)\d)am', r'0\1:00', utter)  # 3am -> 03:00
        utter = re.sub(r'((?<!\d)\d)pm', lambda x: str(int(x.group(1))+12)+':00', utter)  # 3pm -> 15:00
        utter = re.sub(r'(\d+)(:\d+)pm', lambda x: str(int(x.group(1))+12)+x.group(2), utter)  # 5:18pm -> 17:18
        utter = re.sub(r'(\d+)a\.?m',r'\1', utter)  # remove am, a.m
        return utter
...
class DataPreprocessor(object):
    def __init__(self, do_analysis=True):
        ...
        '''
        self.data_path = 'data/MultiWOZ/MULTIWOZ2.1/data_aug.json'
        self.save_path = 'data/MultiWOZ/processed_aug/'
        ...
        self.original_kb_path = 'data/MultiWOZ/MULTIWOZ2.1/knowledge_base.json'
        ...
        self.dialog_acts = json.loads(open('data/MultiWOZ/MULTIWOZ2.1/dialogue_acts_aug.json', 'r').read().lower())
        ...
        self.kb_path = self.save_path + 'kb_processed.json'
        if not os.path.exists(self.kb_path):
            self.preprocess_kb(self.original_kb_path)
    ...
    def analysis(self):
        ...
        # find all reference number, e.g. "reference": "XU1QLHVW"
        ...
        for fn, dial in data.items():
            ...
            for turn in logs:
                if not turn['metadata']:
                    ...
                    meta = turn['metadata']
                    knowledge = turn.get('knowledge')  # get knowledge
                    turn_dict = {'text': turn['text'], 'metadata': {}}
                    # for dom, book_semi in meta.items():
                    for dom, book_semi_sfek in meta.items():
                        # book, semi = book_semi['book'], book_semi['semi']
                        book, semi, sfek = book_semi_sfek['book'], book_semi_sfek['semi'], book_semi_sfek.get("sfek")
                        record = False
                        ...
                        # add seeking for external knowledge
                        if sfek:
                            if not turn_dict['metadata'].get(dom):
                                turn_dict['metadata'][dom] = {}
                            turn_dict['metadata'][dom]['sfek'] = sfek
                    # add knowledge
                    if knowledge:
                        turn_dict['knowledge'] = knowledge
    ...
    def normalize_multiwoz21_name(self):
        ...
        data20 = 'data/MultiWOZ/compressed_data_2.0_aug.json'
    ...
    def preprocess_kb(self, kb_path):
        with open(kb_path, 'r') as f:
            kb = json.loads(f.read().lower())
        for domain, entity_list in kb.items():
            for entity, doc_list in entity_list.items():
                for topic, doc in doc_list.items():
                    kb[domain][entity][topic]["title"] = ' '.join(tknz(clean_text(doc["title"])))
                    kb[domain][entity][topic]["body"] = ' '.join(tknz(clean_text(doc["body"])))
        with open(self.save_path + 'kb_processed.json', 'w', encoding='utf-8') as f:
    ...
    def delexicalization(self, text, dialog_act=None, keep_list=[]):
        ...
        if dialog_act is not None:
            ...
            for act, params in dialog_act.items():
                if 'request' in act or 'general' in act or 'external' in act:
                    continue
                for s_v in params:
        ...
        for fn, raw_dial in tqdm(list(self.original_data.items())):
            ...
            for turn_num, dial_turn in enumerate(raw_dial['log']):
                ...
                if not metadata:   # user
                    ...
                    sk = False
                    sk_dom = []
                    for domain in self.otlg.kb_domains:
                        if metadata[domain].get("sfek"):
                            sk_dom.append(domain)
                            if not dial_state.get(domain):
                                dial_state[domain] = {}
                            en = metadata[domain]["sfek"]
                            dial_state[domain]["sfek"] = en
                            if domain + '-' + 'sfek' not in self.otlg_values:
                                self.otlg_values[domain + '-' + 'sfek'] = [en]
                            elif en not in self.otlg_values[domain + '-' + 'sfek']:
                                self.otlg_values[domain + '-' + 'sfek'].append(en)
                        else:
                            if dial_state.get(domain) and dial_state[domain].get("sfek"):
                                dial_state[domain].pop("sfek")
                    if dial_turn.get('knowledge'):
                        topic.extend(dial_turn["knowledge"]["keywords"])
                        qa_doc["q"] = ' '.join(tknz(clean_text(dial_turn["knowledge"]["snippet"]["title"])))
                        qa_doc["a"] = ' '.join(tknz(clean_text(dial_turn["knowledge"]["snippet"]["body"])))
                        sk = True

                    single_turn['constraint'] = json.dumps(dial_state)
                    single_turn['sk'] = sk
                    single_turn['topic'] = ' '.join(topic)
                    single_turn['qa_doc'] = json.dumps(qa_doc)
                    ...
                    if len(turn_domain) == 2 and sk is not True:
                    ...
                    if 'user' in single_turn:
                        ...
                        delex_vocab = set(single_turn['resp_ori'].split()).difference(set(single_turn['resp'].split()))
                        sk_word = []
                        if sk is True:
                            sk_word = topic + qa_doc["a"].split()
                        for t in single_turn['user'].split() + single_turn['resp'].split() + list(delex_vocab) + sk_word:
